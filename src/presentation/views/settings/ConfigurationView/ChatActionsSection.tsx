...
/** Card of per-browser chat-action visibility toggles. */
export function ChatActionsSection() {
  const { prefs, setPref } = useChatPreferences();

  return (
    <section className="flex flex-col gap-4 rounded-lg border border-border p-4 md:p-6">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Hide advanced affordances on the chat message hover row. Saved to this
          device only.
        </p>
      </div>
