...
test.describe('Smoke — seed-token auth', () => {
  test('seeded token boots straight into authenticated /chat', async ({ page }) => {
    ...
    // The authenticated chat chrome renders: the title-bar "Search chats" button
    // is a stable, UNIQUE signal that ProtectedRoute let us through. (We avoid
    ...
    await expect(page.getByRole('button', { name: /search chats/i })).toBeVisible({
      timeout: 15_000,
    });
