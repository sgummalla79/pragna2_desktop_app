...
## CF-055 — Attachment upload failure is silent: no toast, send not blocked, document silently dropped (nexus-kit-tracker #251)
...
## CF-044 — Agent thinking/reasoning trace vanishes the instant the answer settles (reconcile swap drops the live trace) (nexus-kit-tracker #221)

- **Date:** 2026-06-27
- **Area / file:** `src/presentation/views/chat/utils/carryReasoningAcrossReplace.ts` (new), `src/presentation/views/chat/hooks/useChatSession.ts` (`replaceMessages`). Refs tracker #221. (CF-044 was reserved for this fix at triage; logged now on implementation.)
- **Found by:** Repro — the agent's "thinking"/reasoning umbrella renders live **while streaming** but disappears the instant the run settles. Confirmed in code: the fix the issue prescribed (`carryReasoningAcrossReplace`) did not exist; `replaceMessages` was a bare `agent.setMessages` + `syncMessages` with no trace carry-over.
- **Bug + root cause:** Reasoning is streamed via `reasoning_content` into `reasoningByMessageIdRef`, keyed by the LangChain **stream id** of the in-flight assistant message, so `toChatMessage` renders it live (`reasoningByMessageId.get(m.id)`). On settle, `useReconcileMessages` calls `replaceMessages(initialMessages)` to swap the in-memory list for the persisted snapshot — whose assistant message carries a different **BE UUID** id and no `reasoning`. The per-id lookup then misses on the new id and the trace is dropped. Regressed by commit `4e9cc9a` (CF-005 / TD-030), which added the reconcile-on-settle effect; reasoning had worked since Chat Phase 1 (`5613651`).
- **Fix:** New pure helper `carryReasoningAcrossReplace(prev, replacement, reasoningByMessageId)` re-keys the live trace from each streamed assistant message onto the matching persisted assistant message by **whitespace-normalised content** (mirroring `useReconcileMessages.isCollapsedToolTurn`'s tolerant compare), mutating the same ref `toChatMessage` reads. `replaceMessages` calls it **before** `agent.setMessages`, so the swapped-in BE-UUID message resolves its trace and the umbrella stays visible + expandable for the just-completed turn. Pairing is assistant-only, empty content never matches (no cross-wiring of blank turns), an id that already has a trace is left untouched (idempotent), and first stream-side trace wins per content key. No change needed in `AssistantTurn` — it already folds `m.reasoning` into the umbrella. Tests: `carryReasoningAcrossReplace.test.ts` (8: re-key, whitespace-tolerant pairing, idempotent, empty-content no-match, assistant-only, no-trace no-op, first-wins on duplicate content, multi-turn correct pairing).
- **Web-app applicability:** **CHECK — likely present.** `pragna2_sgummalla_works` shares `useChatSession` + the reconcile-on-settle swap, so the same trace-drop almost certainly reproduces there; port `carryReasoningAcrossReplace` and call it in its `replaceMessages` before the swap. Track under `target:web-fe` (its own session, per the no-cross-repo rule).
